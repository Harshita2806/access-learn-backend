...
import {
    BookOpen, ArrowRight, Sparkles, AlertCircle,
    Headphones, Waves, Users, Library, Heart,
} from 'lucide-react';
...
const containerVariants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.12, delayChildren: 0.3 } } };
const wordVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } };
const badgeVariants = { hidden: { opacity: 0, scale: 0.8 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.5 } } };

export default function LandingPage() {
    return (
        <div className="bg-[#050505] text-white selection:bg-[#14b8a6]/30 h-screen overflow-y-scroll snap-y snap-mandatory scroll-smooth no-scrollbar font-['Nunito_Sans']">
            <style>{`
            ...
            <ProblemSolution />
            <ImpactSlides />
            <div id="features" className="snap-start"><FeatureSection /></div>
            <div id="how-it-works" className="snap-start"><HowItWorks /></div>
            <div className="snap-start"><IntelligentFlowSection /></div>
            <div id="about" className="snap-start"><AboutSection /></div>
...
function Header() {
    ...
    return (
        <motion.header
            initial={{ y: -100 }}
            animate={{ y: 0 }}
            className="fixed top-0 left-0 right-0 z-50 bg-[#050505]/90 backdrop-blur-xl border-b border-[#1a1a1a]"
        >
            <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center gap-8 font-nunito">
                    {["About", "Features", "How It Works", "Contact"].map(name => (
                        <a key={name} href={`#${name.toLowerCase().replace(/ /g, '-')}`} className="text-[#888888] hover:text-[#14b8a6] transition-colors text-sm font-semibold">
                            {name}
                        </a>
                    ))}
                </nav>
                ...
                <div className="flex items-center gap-6 font-nunito">
                    {user ? (
                        <Link to={dashboardPath} className="btn-stealth-teal text-white px-6 py-2 rounded-xl text-sm font-bold">
                            Dashboard
                        </Link>
                    ) : (
                        <>
                            <Link to="/auth" className="text-[#888888] hover:text-white text-sm font-bold transition-colors">Login</Link>
                            <Link to="/auth" className="btn-stealth-teal text-white px-6 py-2 rounded-xl text-sm font-bold">
                                Get Started
                            </Link>
                        </>
                    )}
